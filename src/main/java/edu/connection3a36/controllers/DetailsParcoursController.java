...
public class DetailsParcoursController {
    @FXML private Label lblTitre, lblTypeBadge, lblEtablissement, lblDiplome, lblSpecialite, lblPeriode, lblDescription, lblProjetsCount;
    @FXML private TableView<Projet> tableProjets;
    @FXML private TableColumn<Projet, String> colProjet, colType;
    ...
    public void setParcours(Parcours p) {
        ...
        lblPeriode.setText(p.getDateDebut() + " - " + (p.getDateFin() != null ? p.getDateFin() : "Présent"));
    ...
    @FXML
    private void retour() {
        MainController.getInstance().showBackParcours();
    }
}
