...
public class BackOfficeParcoursController implements Initializable {
    ...
    private void setupColumns() {
        ...
        colTitreInstitution.setCellValueFactory(p -> new javafx.beans.property.SimpleObjectProperty<>(p.getValue().getTitre()));
            ...
            protected void updateItem(String s, boolean e) {
                ...
                if (e)
                    ...
                    if (idx >= 0 && idx < getTableView().getItems().size()) {
                        Parcours p = getTableView().getItems().get(idx);
                        if (p != null) {
                            VBox v = new VBox(2);
        ...
        colTypeDiplome.setCellValueFactory(p -> new javafx.beans.property.SimpleObjectProperty<>(p.getValue().getTypeParcours()));
                    ...
                    if (idx >= 0 && idx < getTableView().getItems().size()) {
                        ...
                        if (p != null) {
                            ...
                            tag.setStyle("-fx-background-color: " + color
                                    + "; -fx-text-fill: white; -fx-padding: 2 8; -fx-background-radius: 10; -fx-font-size: 9;");
        ...
        colPeriode.setCellValueFactory(p -> new javafx.beans.property.SimpleObjectProperty<>(p.getValue().getDateDebut() != null ? p.getValue().getDateDebut().toString() : ""));
        ...
        colProjetsAssocies.setCellValueFactory(p -> new javafx.beans.property.SimpleObjectProperty<>(""));
                    ...
                    if (idx >= 0 && idx < getTableView().getItems().size()) {
                        ...
                        if (p != null) {
                            try {
                                ...
                                l.setStyle(
                                        "-fx-background-color: #f1f5f9; -fx-padding: 3 10; -fx-background-radius: 5; -fx-font-size: 11;");
                                setGraphic(l);
    ...
    public void loadData() {
        try {
            ...
            System.out.println("📊 [DB DEBUG] Fetched " + (data != null ? data.size() : 0) + " parcours from DB.");
            allData.setAll(data != null ? data : new java.util.ArrayList<>());
            ...
            e.printStackTrace();
            Platform.runLater(() -> {
                Alert a = new Alert(Alert.AlertType.ERROR, "Erreur Base de Données : " + e.getMessage());
                a.show();
            });
        }
    }
    ...
    private void updateCharts() {
        ...
        // Sort and Take top 5 for better display
        allData.stream()
                .sorted((p1, p2) -> {
                    try {
                ...
                .forEach(p -> {
                    try {
                        int count = projetService.getByParcoursId(p.getId()).size();
                        String label = p.getTitre().length() > 20 ? p.getTitre().substring(0, 17) + "..."
                                : p.getTitre();
    ...
    private void updateTable() {
        ...
        tableParcours.setItems(FXCollections.observableArrayList(filteredData.subList(from, to)));
        updatePagination();
    }
    ...
    private void updatePagination() {
        ...
        prev.setStyle(
                "-fx-background-color: white; -fx-border-color: #e2e8f0; -fx-border-radius: 5; -fx-padding: 5 15; -fx-cursor: hand;");
        prev.setOnAction(e -> {
            currentPage--;
            updateTable();
        });
        ...
        for (int i = 0; i < totalPages; i++) {
            ...
            if (i == currentPage) {
                b.setStyle(
                        "-fx-background-color: #2563eb; -fx-text-fill: white; -fx-background-radius: 5; -fx-font-weight: bold; -fx-min-width: 35;");
            } else {
                b.setStyle(
                        "-fx-background-color: white; -fx-border-color: #e2e8f0; -fx-border-radius: 5; -fx-text-fill: #64748b; -fx-cursor: hand; -fx-min-width: 35;");
            }
            b.setOnAction(e -> {
                currentPage = idx;
                updateTable();
    ...
    private void injectView(Parent view) {
