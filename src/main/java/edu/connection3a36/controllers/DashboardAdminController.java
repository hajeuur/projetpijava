    ...
    private void loadRecentFeedbacks() {
        ...
        try {
            ...
            for (PlanActions p : feedbacks) {
                VBox box = new VBox(4);
                box.setStyle("-fx-padding: 8; -fx-background-color: #f8f9fa; -fx-border-color: #ecf0f1; -fx-border-radius: 6; -fx-background-radius: 6;");

                Label lblHeader = new Label("Plan #" + p.getId() + " - " + p.getDecision());
                lblHeader.setStyle("-fx-font-weight: bold; -fx-text-fill: #102c59;");

                String fbDate = p.getFeedbackDate() != null ? dtf.format(p.getFeedbackDate()) : "";
                Label lblSub = new Label("Posté le " + fbDate);
                lblSub.setStyle("-fx-font-size: 11px; -fx-text-fill: #7a8fa5;");
